...
:root {
  --bg: #0f0d3a; --card: #17145a; --purple: #322A81; --lime: #8DC63F;
    --amber: #FFC24D; --red: #FF8C8C; --blue: #93C5FD;
      --txt: #e8e6ff; --muted: #a9a4cc; --line: rgba(255,255,255,0.08);
      }
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .btn-sync { background: rgba(141,198,63,.1); color: var(--lime); border: 1px solid rgba(141,198,63,.25); padding: 10px 18px; border-radius: 10px; font-size: 13px; font-weight: 600; cursor: pointer; font-family: 'Inter', sans-serif; transition: .15s; }
      .btn-sync:hover { background: rgba(141,198,63,.18); }
      .btn-disconnect { background: rgba(255,140,140,.08); color: var(--red); border: 1px solid rgba(255,140,140,.2); padding: 10px 18px; border-radius: 10px; font-size: 13px; font-weight: 600; cursor: pointer; font-family: 'Inter', sans-serif; margin-left: 8px; transition: .15s; }
      ...
      .dot-green { background: var(--lime); box-shadow: 0 0 6px rgba(141,198,63,.5); }
...
function SettingsContent() {
  ...
  useEffect(() => {
        ...
        const user = searchParams.get('user');
                ...
                if (connected === 'outlook') {
                        ...
                        showToast(`Procore ${en ? 'connected' : 'conectado'}${user ? ` - ${user}` : ''}`);
                        fetchStatus();
                } else if (error) {
  ...
  const disconnectOutlook = async () => {
        if (!confirm(en ? 'Disconnect Outlook? The access token will be lost.' : 'Desconectar Outlook? Se perdera el token de acceso.')) return;
  ...
  const syncNow = async () => {
        ...
        try {
                ...
                if (d.success) {
                          ...
                          showToast(d.error || (en ? 'Sync error' : 'Error en sincronizacion'), 'error');
                }
        } catch {
  ...
  const steps = en ? [
    { n:'1', t:'Secure OAuth Login', d:'Microsoft / Procore ask you to authorize JAXI - tokens are stored encrypted locally.' },
    { n:'2', t:'Initial Import', d:'All your active projects, RFIs and Submittals are fetched from Procore.' },
    ...
    { n:'1', t:'Login OAuth seguro', d:'Microsoft / Procore te piden autorizar JAXI - los tokens se guardan encriptados localmente.' },
    { n:'2', t:'Importacion inicial', d:'Se traen todos tus proyectos, RFIs y Submittals activos de Procore.' },
              ...
              <div className="wrap">
                      <div style={{ padding: '24px 0 0', marginBottom: 36 }}>
                                <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:8 }}>
                                            ...
                                            <h1>{en ? 'Connection Settings' : 'Configuracion de Conexiones'}</h1>h1>
                                </div>div>
                                <p className="sub">
                                  {en ? 'Connect Microsoft Outlook and Procore to get real-time data. Tokens are stored securely in the local database.' : 'Conecta Microsoft Outlook y Procore para obtener datos en tiempo real. Los tokens se guardan de forma segura en la base de datos local.'}
                                </p>p>
                      </div>div>
                ...
                {/* Microsoft Outlook */}
                      <div className="card">
                                <div className="card-head">
                                            ...
                                            <h2>Microsoft Outlook</h2>h2>
                                  {status === null ? <span className="badge badge-loading">{en ? 'Checking...' : 'Verificando...'}</span>span> : outlookOk ? <span className="badge badge-connected">CONNECTED</span>span> : <span className="badge badge-warn">{en ? 'Not Connected' : 'Sin conexion'}</span>span> }
                                </div>div>
                                <p className="desc">
                                  {en ? <>Reads project-related emails from your Outlook, automatically detects commitments (<em>'I will reply Friday'</em>em>) and cross-references them with Procore RFIs/Submittals. You need credentials in <code>backend/.env</code>code> first.</>> : <>Lee emails de tu Outlook relacionados con proyectos, detecta compromisos automaticamente (<em>'te respondo el viernes'</em>em>) y los cruza con los RFIs/Submittals de Procore. Necesitas las credenciales en <code>backend/.env</code>code> primero.</>>}
                                </p>p>
                        {outlookOk ? (
                      <>
                                    <div className="stat-row">
                                                    ...
                                                    <span>
                                                                      <strong>{emailCount}</strong>strong> {en ? 'email threads imported' : 'email threads importados'} {lastSync && ` . ${en ? 'Last sync' : 'Ultima sync'}: ${new Date(lastSync).toLocaleString(en ? 'en-US' : 'es-US')}`}
                                                    </span>span>
                                    </div>div>
                                    <div className="btn-row">
                                                    <button className="btn-sync" onClick={syncNow} disabled={syncing}>
                      ...
                      <>
                                    <div className="env-block">
                                                    <div>MICROSOFT_CLIENT_ID={en ? 'your_azure_client_id' : 'tu_azure_client_id'}</div>div>
                                                    <div>MICROSOFT_CLIENT_SECRET={en ? 'your_client_secret' : 'tu_client_secret'}</div>div>
                                                    <div>MICROSOFT_TENANT_ID=common</div>div>
                                                    <div>MICROSOFT_REDIRECT_URI=http://localhost:3001/api/v1/integrations/outlook/callback</div>div>
                                    </div>div>
                ...
                {/* Procore */}
                      <div className="card">
                                <div className="card-head">
                                            ...
                                            <h2>Procore</h2>h2>
                                  {status === null ? <span className="badge badge-loading">{en ? 'Checking...' : 'Verificando...'}</span>span> : procoreOk ? <span className="badge badge-connected">CONNECTED</span>span> : <span className="badge badge-warn">{en ? 'Not Connected' : 'Sin conexion'}</span>span> }
                                </div>div>
                                <p className="desc">
                                  {en ? <>Import projects, RFIs and Submittals directly from your Procore account. Data updates automatically. You need credentials from the <strong>Procore Developer Portal</strong>strong>.</>> : <>Importa proyectos, RFIs y Submittals directamente desde tu cuenta de Procore. Los datos se actualizan automaticamente. Necesitas credenciales del <strong>Procore Developer Portal</strong>strong>.</>>}
                                </p>p>
                        {procoreOk ? (
                      <div className="stat-row">
                                    <span className="stat-dot dot-green" />
                                    <span>{en ? 'Procore connected - syncing RFIs and Submittals' : 'Procore conectado - sincronizando RFIs y Submittals'}</span>span>
                      </div>div>
                    ) : (
                      <>
                                    <div className="env-block">
                                                    <div>PROCORE_CLIENT_ID={en ? 'your_procore_client_id' : 'tu_procore_client_id'}</div>div>
                                                    <div>PROCORE_CLIENT_SECRET={en ? 'your_procore_secret' : 'tu_procore_secret'}</div>div>
                                                    <div>PROCORE_REDIRECT_URI=http://localhost:3001/api/v1/integrations/procore/callback</div>div>
                                    </div>div>
                                    <button className="connect-btn btn-pc" style={{ marginTop:16 }} onClick={connectProcore}>
                                ...
                                <div className="card-head">
                                            <div style={{ width:38, height:38, borderRadius:9, background:'rgba(159,211,64,0.10)', border:'1px solid rgba(159,211,64,0.20)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                                                          <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="#9FD340" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                                                                          ...
                                                                          <circle cx="5" cy="12" r="2" fill="#9FD340" stroke="none"/>
                                                                          <circle cx="12" cy="5" r="2" fill="rgba(159,211,64,0.4)" stroke="none"/>
                                                                          <circle cx="12" cy="19" r="2" fill="rgba(159,211,64,0.4)" stroke="none"/>
                                                                          <circle cx="19" cy="12" r="2" fill="#9FD340" stroke="none"/>
                                                          </svg>svg>
                                            </div>div>
                                            <h2>{en ? 'What happens when you connect?' : 'Que pasa cuando conectas?'}</h2>h2>
                                </div>div>
                                <div style={{ display:'flex', flexDirection:'column', gap:14, marginTop:8 }}>
                                  {steps.map((s, idx) => {
                        const stepIcons = [
                                          // 1. OAuth lock
                                          <svg key="1" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>svg>,
                                          // 2. Download / import
                                          <svg key="2" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>svg>,
                                          // 3. Cross-reference / link
                                          <svg key="3" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>svg>,
                                          // 4. Live / activity
                                          <svg key="4" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>svg>,
                                        ];
                        return (
                                          <div key={s.n} style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
                                                            <div style={{ background:'var(--lime)', color:'#0f0d3a', borderRadius:8, width:28, height:28, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:800, fontSize:13, flexShrink:0 }}>
                                                              {stepIcons[idx]}
                                                            </div>div>
                                                            <div>
                                                                                <div style={{ fontWeight:600, fontSize:14, color:'var(--txt)' }}>{s.t}</div>div>
                                                                                <div style={{ fontSize:13, color:'var(--muted)', marginTop:2, lineHeight:1.5 }}>{s.d}</div>div>
                                                            </div>div>
                                          </div>div>
                                        );
        })}
