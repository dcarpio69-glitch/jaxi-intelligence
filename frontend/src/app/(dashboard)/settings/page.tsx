...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (connected === 'outlook') {
      ...
      const msgs: Record<string, Record<string, string>> = {
        outlook_auth_failed:  { es: 'Error de autenticación con Outlook',    en: 'Outlook authentication failed' },
        outlook_token_failed: { es: 'Error al obtener token de Outlook',     en: 'Failed to get Outlook token' },
        outlook_no_code:      { es: 'No se recibió código de autorización',  en: 'Authorization code not received' },
        procore_auth_failed:  { es: 'Error de autenticación con Procore',    en: 'Procore authentication failed' },
  ...
  const disconnectOutlook = async () => {
    if (!confirm(en ? 'Disconnect Outlook? The access token will be lost.' : '¿Desconectar Outlook? Se perderá el token de acceso.')) return;
  ...
  const syncNow = async () => {
    ...
    try {
      ...
      const d = await r.json();
      if (d.success) {
        showToast(en ? `✓ Sync complete — ${d.emailsSynced} emails imported` : `✓ Sinc completada — ${d.emailsSynced} emails importados`);
        fetchStatus();
      } else {
    ...
    <>
      ...
      <div className="wrap">
        <div style={{ padding: '24px 0 0', marginBottom: 36 }}>
          ...
          <p className="sub">
            {en
              ? 'Connect Microsoft Outlook and Procore to get real-time data. Tokens are stored securely in the local database.'
              : 'Conecta Microsoft Outlook y Procore para obtener datos en tiempo real. Los tokens se guardan de forma segura en la base de datos local.'}
          </p>
        </div>
        ...
        <div className="card">
          <div className="card-head">
            ...
            {status === null
              ...
              : outlookOk
                ? <span className="badge badge-connected">✓ {en ? 'CONNECTED' : 'CONECTADO'}</span>
                : <span className="badge badge-warn">{en ? 'Not Connected' : 'Sin conexión'}</span>
            }
          </div>
          <p className="desc">
            {en
              ? <>Reads project-related emails from your Outlook, automatically detects commitments (<em>"I'll reply Friday"</em>) and cross-references them with Procore RFIs/Submittals. You need credentials in <code>backend/.env</code> first.</>
              : <>Lee emails de tu Outlook relacionados con proyectos, detecta compromisos automáticamente (<em>"te respondo el viernes"</em>) y los cruza con los RFIs/Submittals de Procore. Necesitas las credenciales en <code>backend/.env</code> primero.</>}
          </p>
          {outlookOk ? (
            <>
              <div className="stat-row">
                ...
                <span>
                  <strong>{emailCount}</strong> {en ? 'email threads imported' : 'email threads importados'}
            ...
            <>
              <div className="env-block">
                ...
                <div>MICROSOFT_REDIRECT_URI=http://localhost:3001/api/v1/integrations/outlook/callback</div>
              </div>
        ...
        <div className="card">
          ...
          <p className="desc">
            {en
              ? <>Import projects, RFIs and Submittals directly from your Procore account. Data updates automatically. You need credentials from the <strong>Procore Developer Portal</strong>.</>
              : <>Importa proyectos, RFIs y Submittals directamente desde tu cuenta de Procore. Los datos se actualizan automáticamente. Necesitas credenciales del <strong>Procore Developer Portal</strong>.</>}
          </p>
          {procoreOk ? (
            <div className="stat-row">
              <span className="stat-dot dot-green" />
              <span>{en ? 'Procore connected — syncing RFIs and Submittals' : 'Procore conectado — sincronizando RFIs y Submittals'}</span>
            </div>
          ) : (
            <>
              <div className="env-block">
                <div>PROCORE_CLIENT_ID={en ? 'your_procore_client_id' : 'tu_procore_client_id'}</div>
                <div>PROCORE_CLIENT_SECRET={en ? 'your_procore_secret' : 'tu_procore_secret'}</div>
                <div>PROCORE_REDIRECT_URI=http://localhost:3001/api/v1/integrations/procore/callback</div>
              </div>
        ...
        <div className="card">
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14, marginTop:8 }}>
            {steps.map((s, idx) => {
              const stepIcons = [
                // 1. OAuth lock
                <svg key="1" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>,
                // 2. Download / import
                <svg key="2" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>,
                // 3. Cross-reference / link
                <svg key="3" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>,
                // 4. Live / activity
                <svg key="4" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>,
              ];
              return (
                <div key={s.n} style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
                  ...
                  <div>
                    <div style={{ fontWeight:600, fontSize:14, color:'var(--txt)' }}>{s.t}</div>
                    <div style={{ fontSize:13, color:'var(--muted)', marginTop:2, lineHeight:1.5 }}>{s.d}</div>
                  </div>
                </div>
              );
            })}
